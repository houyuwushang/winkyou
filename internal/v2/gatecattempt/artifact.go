...
const (
	ArtifactProfile           = "winkyou-direct-oob-attempt/1"
	ManifestProfile           = "winkyou-direct-oob-manifest/1"
	DirectAttemptProfile      = "winkyou-direct-hard-nat-control/1"
	OOBCarrierProfile         = "ssh-bounded-child-stream/1"
	ObservationProfile        = "rfc5780-allocation-tomography/1"
	DataPlaneConsumerProfile  = "wireguard-direct-session/1"
	DataPlaneChallengeProfile = "wireguard-handshake-echo/1"
	ProtocolVersion           = "winkyou-direct-pairing/1"
	AuthScope                 = "operator-initiated-one-shot/1"
	ObservationGeneration     = "1"
	NoiseBindingLabel         = "winkyou product direct OOB pairing v1\n"
	MaxArtifactBytes          = 4096
	MaxManifestBytes          = 4096
)

var (
	ErrInvalidArtifact       = errors.New("gatecattempt: invalid artifact")
	ErrUnsupportedProfile    = errors.New("gatecattempt: unsupported profile")
	ErrCredentialNotYetValid = errors.New("gatecattempt: credential not yet valid")
	ErrCredentialExpired     = errors.New("gatecattempt: credential expired")
	ErrSecretUnavailable     = errors.New("gatecattempt: pairing secret unavailable")
	ErrInvalidManifest       = errors.New("gatecattempt: invalid manifest")
)
...
type wireArtifact struct {
	...
	DataPlaneConsumerProfile  string        `json:"data_plane_consumer_profile"`
	DataPlaneChallengeProfile string        `json:"data_plane_challenge_profile"`
...
// Artifact contains no endpoint, hostname, username, command, path, target,
// candidate list, observer address, WireGuard identity, or resource count.
...
func validateWire(wire *wireArtifact, now time.Time) (*Artifact, error) {
	...
	if wire.Artifact != ArtifactProfile || wire.DirectAttemptProfile != DirectAttemptProfile ||
		...
		wire.DataPlaneConsumerProfile != DataPlaneConsumerProfile || wire.DataPlaneChallengeProfile != DataPlaneChallengeProfile ||
		wire.Protocol != ProtocolVersion || wire.SecureChannelProfile != pairingcontext.SelectedSecureChannelProfile {
	...
	if !localRole.Valid() || !validPlannerRoles(profile, initiatorPlanner, responderPlanner) ||
		wire.AuthScope != AuthScope || wire.ObservationGeneration != ObservationGeneration ||
	...
	canonical, err := canonicalContext(contextValue)
	if err != nil {
		return nil, ErrInvalidArtifact
	}
...
func (artifact *Artifact) NoisePrologue() ([]byte, error) {
	...
	canonical, err := canonicalContext(artifact.context)
	if err != nil {
		return nil, err
	}
	binding := NoiseBindingLabel +
		...
		"consumer=" + DataPlaneConsumerProfile + "\n" +
		"challenge=" + DataPlaneChallengeProfile + "\n" +
	...
	result := make([]byte, 0, len(binding)+len(canonical))
...
func (artifact *Artifact) GateBArtifactKind() string { return "gate-c-product-artifact/1" }
...
func EncodeArtifactSet(material ArtifactMaterial, psk [32]byte) (set ArtifactSet, err error) {
	...
	base := wireArtifact{
		...
		DataPlaneConsumerProfile: DataPlaneConsumerProfile, DataPlaneChallengeProfile: DataPlaneChallengeProfile,
		OOBChannelID: material.OOBChannelID, InitiatorPlannerRole: string(material.InitiatorPlannerRole),
		ResponderPlannerRole: string(material.ResponderPlannerRole), Protocol: ProtocolVersion, AuthScope: AuthScope,
		CredentialID: material.CredentialID, AttemptID: material.AttemptID, ObservationGeneration: ObservationGeneration,
		...
		RuntimeFallback: pairingcontext.FeatureDisabled, IssuedAt: issued.Format(time.RFC3339), ExpiresAt: expires.Format(time.RFC3339),
	}
	...
	encode := func(role Role) ([]byte, error) {
		wire := generatedWire{
			...
			DataPlaneConsumerProfile: base.DataPlaneConsumerProfile, DataPlaneChallengeProfile: base.DataPlaneChallengeProfile,
	...
	manifest := Manifest{
		...
		OOBCarrierProfile: OOBCarrierProfile, ObservationProfile: ObservationProfile,
		DataPlaneConsumerProfile: DataPlaneConsumerProfile, DataPlaneChallengeProfile: DataPlaneChallengeProfile,
		SecureChannelProfile: pairingcontext.SelectedSecureChannelProfile, AuthScope: AuthScope,
...
func ParseManifest(payload []byte) (Manifest, error) {
	...
	if manifest.Schema != ManifestProfile || manifest.ArtifactProfile != ArtifactProfile ||
		...
		manifest.ObservationProfile != ObservationProfile || manifest.DataPlaneConsumerProfile != DataPlaneConsumerProfile ||
		manifest.DataPlaneChallengeProfile != DataPlaneChallengeProfile ||
		manifest.SecureChannelProfile != pairingcontext.SelectedSecureChannelProfile || manifest.AuthScope != AuthScope ||
		manifest.RuntimeFallback != pairingcontext.FeatureDisabled || !validBase64URL(manifest.CredentialID, 16) ||
		!validBase64URL(manifest.AttemptID, 16) || !validBase64URL(manifest.OOBChannelID, 16) ||
		!validBase64URL(manifest.ArtifactFingerprint, sha256.Size) {
		return Manifest{}, ErrInvalidManifest
	}
	if _, err := hardnatbudget.For(profile, resource); err != nil {
		return Manifest{}, ErrInvalidManifest
...
func canonicalContext(context pairingcontext.PairingContext) ([]byte, error) {
	if context.Artifact != pairingcontext.PairingArtifactAcceptance || context.Protocol != ProtocolVersion ||
		context.AuthScope != AuthScope || context.ObservationGeneration != ObservationGeneration ||
		context.InitiatorGovernorScope != pairingcontext.GovernorScopeMachine ||
		context.ResponderGovernorScope != pairingcontext.GovernorScopeMachine ||
		context.SecureChannelProfile != pairingcontext.SelectedSecureChannelProfile ||
		context.InitiatorChannelRole != pairingcontext.ChannelRoleInitiator ||
		context.ResponderChannelRole != pairingcontext.ChannelRoleResponder ||
		context.EarlyData != pairingcontext.FeatureDisabled || context.Resumption != pairingcontext.FeatureDisabled ||
		context.RuntimeFallback != pairingcontext.FeatureDisabled {
		return nil, ErrInvalidArtifact
	}
	return pairingcontext.CanonicalizeFlatStringObject(context.Object())
}
...
func artifactFingerprint(wire *wireArtifact) (string, error) {
	...
	object := map[string]any{
		...
		"data_plane_consumer_profile":  wire.DataPlaneConsumerProfile,
		"data_plane_challenge_profile": wire.DataPlaneChallengeProfile,
		...
		"credential_id": wire.CredentialID, "attempt_id": wire.AttemptID,
		...
		"responder_channel_role": wire.ResponderChannelRole, "early_data": wire.EarlyData,
		"resumption": wire.Resumption, "runtime_fallback": wire.RuntimeFallback,
		"issued_at": wire.IssuedAt, "expires_at": wire.ExpiresAt,
	}
