...
func TestResolveListenAndRelayBindAddressLocalInterfaceOverride(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotListen, gotBind, err := resolveListenAndRelayBindAddress(tt.listen, tt.relayIP, tt.allowWildcard, func(net.IP) bool {
				return tt.hasLocalIP
			})
			if err != nil {
				t.Fatalf("resolveListenAndRelayBindAddress() error = %v", err)
			}
