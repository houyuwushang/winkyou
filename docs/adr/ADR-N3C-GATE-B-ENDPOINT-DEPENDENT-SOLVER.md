...
> 2026-09-06 新增待审问题：[#106](https://github.com/houyuwushang/winkyou/issues/106) /
> [诊断 PR #107](https://github.com/houyuwushang/winkyou/pull/107) 复现了早期命中在最终确认前
> 失去 kernel flow 的机制；[映射寿命与提前确认草案](./ADR-N3C-HARD16-MAPPING-LIFETIME.md)
> 单独列出模型、终局与协议待决项。该 Draft 不覆盖本 ADR 的预算、wire 或 §20/§22 规则，
> 不授权修实现、放宽测试、C1c 或现场 I/O。
