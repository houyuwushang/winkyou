...
| 待决项 | 维护者 / 独立评审裁决 |
| --- | --- |
| P1 跨版本密文同一要求的最小修订 | 搁置期间不裁决。独立复审意见（非裁决，2026-09-14）：若日后重启 E2，倾向接受“`/1` parser/golden/行为原样保留，E2 作为 `/2` 另加”，不承诺与 `/1` 密文相同。 |
| P2 晚到包与 syscall 见证的可证明边界 | 搁置期间不裁决。独立复审意见（非裁决）：倾向接受“合法但晚到的 candidate 仅丢弃、不产生新 proposal”，并明确区分本地 commit 见证与远端可验证材料，不虚构“远端 stop 后零发包”的证明。 |
| 是否接受 E2 / 改走 E1 / 搁置 | **搁置**（维护者，2026-09-14）。重启条件：Gate C1c 一次性路由器现场数据——每次命中 tuple 的映射年龄、stop→W→VERIFY 用时、失败阶段（字段名在 C1c mini-spec 冻结）。若真实 NAT 映射寿命普遍覆盖 full schedule + 确认耗时，则 E 不需要；若接近 M-E 的 30s 模型，则重启 E2 评审。不切 E1，不做运行时降级。 |
| 是否接受标识、仲裁、128-byte pending W 与向量结构 | 保留为草案，未接受。 |
| 是否授权实现；exact SHA / 范围 / 日期 | 否。 |

搁置不改变 §1 的容量结论，也不改变 §7 的可用性限制声明：现场必须如实记录寿命与失败阶段，
不能声称所有困难 NAT 都能穿透。本文仍为 Draft，不改变任何已合入二进制、网络权限或现场前置。
