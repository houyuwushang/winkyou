...
- Accepted C1c prerequisite session-liveness re-adjudication (decisions filled 2026-09-06 by the independent reviewer under maintainer delegation): retain WireGuard, bounded bilateral in-tunnel challenges (K=20s/R=5s/M=2|3), per-write expiry and separate control budgets; the text closes the §16.8 gate only — liveness implementation, C1c and field I/O each still require separate maintainer authorization and review: [`adr/ADR-N3C-SESSION-LIVENESS.md`](./adr/ADR-N3C-SESSION-LIVENESS.md)
