...
- `candidate_failed` 中的 `last_local_candidate_*` / `last_remote_candidate_*` / `last_signal_*`：这是失败事件携带的最近一次候选和候选信令摘要。如果 `last_local_candidate_kept=0`，优先查本端 STUN、hint、CIDR 过滤和本地 base；如果 `last_remote_candidate_kept=0`，优先查对端是否发布了公网/受信 underlay 候选，或本端是否缺少对应 `nat.direct_trusted_cidrs`。如果 `last_signal_candidate_sent` 为空，说明本端尚未走到 public-direct candidate signal；如果已发送多轮但远端仍没有 usable candidate，继续查控制面延迟、两端版本和远端过滤摘要。
