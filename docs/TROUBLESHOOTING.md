...
- doctor 的 `candidate filters` 会显示静态配置的 `public_endpoint_hint_port_window`。如果当前 STUN probe 判定为 symmetric NAT，或 NAT 类型无法分类但本轮能生成 endpoint hint，还会额外显示 `effective_public_endpoint_hint_port_window=512` 和 `effective_window_reason=symmetric_nat_endpoint_hints` / `unclassified_nat_endpoint_hints`，表示生产策略会使用放宽后的有效端口窗口。
- `candidate_signaled`：`public_direct` 在 offer/answer 后额外发送的有界 candidate 信令。重点看 `candidate_sent`、`candidate_total`、`candidate_round` / `candidate_rounds`、`retry_interval_ms`、`signal_window_ms`、`send_timeout_ms` 和 `candidate_capped`；如果 `candidate_capped=true`，当前实现会优先发送 `public_endpoint_hints`/端口窗口生成的候选。如果完全没有该事件，优先确认两端二进制是否包含候选信令重发补强。
- 第一轮 `candidate_signaled` 会随 offer/answer 立即发送，后续 round 是后台 retry；默认是短间隔三轮，生产路径会按 `nat.connect_timeout` 拉长 retry 间隔并最多发送二十轮。`signal_window_ms` 太短时，说明两端可能仍在短窗口内错过候选；如果只看到第一轮但没有后续轮次，继续查 executor 是否已关闭、ctx 是否取消或信令发送是否报错。
- `remote_candidates_waiting`：offer/answer credentials 已到达但候选全被过滤时，public-direct 会继续等待后续 `candidate` burst。重点看 `candidate_grace_ms`；生产路径会按 `nat.connect_timeout` 拉长该窗口，但仍有上限，避免彻底阻塞后续 fallback。
...
- 如果早到 `candidate` 是有效 public-direct 候选，executor 会在缓存前先从 same-socket public-direct puncher 做一次 best-effort pre-punch，但仍不会把候选加入 ICE agent，也不会在 remote credentials 到达前启动 ICE connect。这用于缩短乱序信令下的 NAT 打洞窗口。
- `candidate_failed` 中的 `last_local_candidate_*` / `last_remote_candidate_*` / `last_signal_*`：这是失败事件携带的最近一次候选和候选信令摘要。如果 `last_local_candidate_kept=0`，优先查本端 STUN、hint、CIDR 过滤和本地 base；如果 `public_endpoint_hint_local_base_count=0`，说明本轮 hint 没有绑定具体本地 UDP base；如果有 `public_endpoint_hint_fixed_local_port` 和 `public_endpoint_hint_fixed_udp_mux_candidate=true`，说明本轮具备固定本地端口 mux 条件，可继续排查公网端点和端口窗口是否覆盖 natpierce 观测值。如果 `last_remote_candidate_kept=0`，优先查对端是否发布了公网/受信 underlay 候选，或本端是否缺少对应 `nat.direct_trusted_cidrs`。如果 `last_signal_candidate_sent` 为空，说明本端尚未走到 public-direct candidate signal；如果已发送多轮但远端仍没有 usable candidate，继续查控制面延迟、两端版本和远端过滤摘要。`ice_remote_candidate_count` 表示候选是否已经进到 ICE agent；`ice_selected_pair=false` 且 `ice_state=checking/failed` 通常说明候选已交换但 ICE checks 没选中路径。
- `candidate_failed` 中的 `last_punch_*`：这是失败事件携带的最近一次 same-socket pre-punch 摘要。重点看 `last_punch_punch_local_addr`、`last_punch_candidate_first`、`last_punch_candidate_last`、`last_punch_candidate_port_min/max`、`last_punch_candidate_start/next_start` 和 `last_punch_punch_round/rounds`，确认失败前是否真的从固定 socket 覆盖了期望端口窗口。
- `remote_candidates_punched` / `candidate_failed.last_punch_*` 中的 `packet_sent` 和 `punch_burst`：public-direct pre-punch 会对每个远端候选发送受控多包 burst，而不只是单个 UDP 包。`candidate_sent` 表示覆盖了多少个候选端点，`packet_sent` 表示实际打出的 UDP punch 包数；如果 `candidate_sent>0` 但 `packet_sent` 很低，优先查本端 socket、上下文超时或系统 UDP 发送限制。
