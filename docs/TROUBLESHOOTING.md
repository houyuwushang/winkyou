...
- `candidate_signaled`：`public_direct` 在 offer/answer 后额外发送的有界 candidate 信令。重点看 `candidate_sent`、`candidate_total`、`candidate_round` / `candidate_rounds` 和 `candidate_capped`；如果 `candidate_capped=true`，当前实现会优先发送 `public_endpoint_hints`/端口窗口生成的候选。如果完全没有该事件，优先确认两端二进制是否包含候选信令重发补强。
