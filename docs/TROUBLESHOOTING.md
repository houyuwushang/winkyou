...
- `candidate_signaled`：`public_direct` 在 offer/answer 后额外发送的有界 candidate 信令。重点看 `candidate_sent`、`candidate_total`、`candidate_round` / `candidate_rounds`、`send_timeout_ms` 和 `candidate_capped`；如果 `candidate_capped=true`，当前实现会优先发送 `public_endpoint_hints`/端口窗口生成的候选。如果完全没有该事件，优先确认两端二进制是否包含候选信令重发补强。
- 第一轮 `candidate_signaled` 会随 offer/answer 立即发送，后续 round 是后台 retry；如果只看到第一轮但没有后续轮次，继续查 executor 是否已关闭、ctx 是否取消或信令发送是否报错。
