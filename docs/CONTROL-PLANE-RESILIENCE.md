...
- 已建立虚拟网后的 in-band peer control channel 已接入 heartbeat/path_health、最小 re-ICE request 和 session_signal 冗余信令；完整 endpoint/capability/observation 交换 UX、可靠性/backoff 和真实故障验证仍待实现。
...
- 已承载 heartbeat/path health、最小 re-ICE request 和 session_signal 冗余信令；后续再接 endpoint update、capability refresh、observation exchange、可靠重传或更完整的恢复策略。
