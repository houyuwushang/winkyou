...
| 2 离线能力检查 | 核实OS/内核/UID/TUN/工具、独立管理路径、router两NAT域及observer双地址双端口权限；核对namespace与全局ceiling的不同管理域 | `uname -r`；`id -u`；`test -c <TUN_DEVICE>`；`conntrack -V`；`<READ_ONLY_CAPABILITY_CHECK>` | 能力/许可不足即停止；不换公网observer，不降低隔离要求。 |
