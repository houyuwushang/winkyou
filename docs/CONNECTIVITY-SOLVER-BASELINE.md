...
Client runtime must distinguish coordinator/control-plane state from data-plane state. The first peer-offline cleanup guard is already in place: a peer with recent WireGuard handshake evidence, packet counters, and no transport error should not be removed only because a peer update reports it offline. Runtime state now exposes `control_state`, `data_state`, and recent successful path cache fields for operator visibility. This does not yet complete control-plane resilience. Heartbeat failure, signaling stream failure, cached path recovery, and real coordinator-process outage validation remain v0.1 hardening work.
