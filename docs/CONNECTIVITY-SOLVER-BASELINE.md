...
- `signal_relay` (low-throughput coordinator signal fallback)

Production registration remains compatible by default: `legacy_ice_udp` first, then `relay_only`, then `signal_relay`. `tcp_framed` is disabled by default and is registered only when `tcp_framed.enabled=true` and it appears in `connectivity.strategy_order`.

`tcp_framed` is an explicit-TCP alpha strategy, not TCP NAT hole punching. Its default role is `auto`, preserving the old behavior where the initiator listens and the controlled side dials. Operators may set `tcp_framed.role=listen` or `tcp_framed.role=dial` to override that direction, and `tcp_framed.dial_addr` may be used to dial a fixed reachable endpoint without waiting for an offer. Static endpoint dialing should retry within `tcp_framed.dial_timeout` so a listener that comes up slightly later during simultaneous peer startup is still usable. This is intended for known reachable TCP listeners, port-forwarded endpoints, or controlled lab overlays. A successful SSH or management connection through an external overlay, such as natpierce, does not prove that random `tcp_framed` ports are reachable; the advertised or static TCP endpoint itself must be reachable from the peer. `tcp_framed` path summaries must classify the remote TCP endpoint conservatively: public endpoints may be dependency-free `protected_direct`, while private, CGN/100.64, loopback, link-local, benchmark/overlay, or otherwise ambiguous endpoints remain `primary_candidate` with an `unknown` dependency unless they match explicit operator trust in `nat.direct_trusted_cidrs` or the compatibility `nat.public_direct_trusted_cidrs`.

`wink doctor --strategy tcp_framed` should expose the effective `tcp_framed` role/listen/advertise/dial settings. When `tcp_framed.dial_addr` is configured, doctor may perform a bounded TCP connect probe to that endpoint so operators can distinguish "configured" from "currently reachable".

`signal_relay` adapts the existing coordinator strategy-message stream into a `PacketTransport`. It sends already-encrypted WireGuard packets as strategy messages, reports `connection_type=relay`, and marks the path dependency as `coordinator:<peer>:coordinator_signal_stream`. This is a low-throughput fallback and diagnostic keepalive path for environments where TURN is unavailable and direct/TCP attempts are still being improved. It is not a TURN replacement, it is not protected direct evidence, and it cannot survive loss of the coordinator signal stream that carries it.
...
- `legacyice/public_direct`: gather only host/server-reflexive direct candidates, skip TURN/relay gathering, then advertise only public direct candidates and ignore ambiguous remote candidates, including private ranges, `100.64.0.0/10`, loopback, link-local, multicast, and benchmark/overlay ranges, before attempting a public direct ICE path. The public-direct ICE agent must also avoid likely external overlay interfaces by name by default, including natpierce, Tailscale, ZeroTier, Docker/vEthernet, Wintun/WinkYou, and similar virtual adapters; an explicit `nat.candidate_interface_include` is an operator opt-in override for controlled comparison, while explicit `nat.candidate_interface_exclude` still wins. Ordinary `direct_prefer` may still use those paths as a best-effort connectivity candidate, but they must not prove protected direct unless the operator has also supplied a narrow trusted underlay policy. A configured UDP TURN URL may be converted into an unauthenticated STUN binding URL on the same host/port for srflx gathering, but TURN relay allocation and relay candidates remain disabled in this plan. Operator-supplied `nat.public_endpoint_hints` may add local public UDP `ip:port` values as extra public-direct srflx candidates, and may bind them to a concrete local UDP base using `public_ip:public_port/local_ip:local_port`. By default, the client merges startup STUN mapping observations into those hints as best-effort candidates, including endpoint-dependent or unclassified observations that should still be compared with natpierce/router evidence instead of treated as proven stable. `nat.public_endpoint_hint_port_window` is a bounded expansion around each hinted public port for predictable port drift; the configured default is small, but production may raise the effective window when endpoint hints exist and the startup NAT type is symmetric or unclassified/unknown, while preserving an explicit zero opt-out and larger user values. This remains at the NAT/legacy ICE boundary. When those mapped hints include local base addresses, the public-direct ICE agent should restrict gathering to those local IPs; when they contain exactly one unique local base port, it should create a fixed UDP mux on that local `ip:port` so host, STUN/server-reflexive candidates, and best-effort remote-candidate pre-punch share the same socket and the advertised hint matches the packet source used for punching; when the mapped public port differs from that local base port, the same public IP plus local base port should also be used as a second bounded prediction center for port-preserving or endpoint-dependent NAT behavior; when they contain multiple local base ports, the strategy should split them into separate public-direct hint plans and bind each attempt to its corresponding local base port. Split hint plans remain in the same `legacyice/public_direct` signaling family so peers with different local hint counts can still exchange offer/answer messages, and session should execute contiguous same-family hint plans concurrently inside one candidate window. Base-family public-direct messages may be delivered to all active hint executors, while exact `public_direct_hint_N` messages should prefer the matching executor to avoid unnecessary credential mixing. A successful hint should cancel slower same-family hint executors and proceed to binding; all failed hints must still leave relay or later fallback plans executable. This is useful when another tool or router log has already observed usable public endpoints for concrete local sockets. After sending an offer or answer, `public_direct` may also send a bounded burst of `candidate` messages for the same candidates. The first candidate round should be sent immediately; retry rounds may continue in the executor lifecycle background, scale with `nat.connect_timeout` up to a hard cap, scale each send window with the per-round candidate count up to a hard cap, and must not delay ICE connect. These messages are redundant with the full offer/answer candidate list, must be capped per round to avoid flooding large port windows, and must prioritize public endpoint hint/window candidates before lower-value candidates when capped. Multiple endpoint hints should be ordered by offset distance first and hint index second so candidate caps cover low-offset ports for every hint instead of exhausting one hint's full window before trying the next. They must not start ICE checks before remote credentials are available if they arrive out of order; valid early candidates should be buffered, deduplicated, and merged with the later offer/answer candidates after credentials arrive so a sparse or filtered answer does not discard a short-window punch candidate. If remote credentials arrive first but all offer/answer candidates are filtered, the public-direct executor should wait briefly for follow-up `candidate` messages before failing the plan; a `remote_candidates_waiting` observation should make that grace period visible. If public-direct gathering times out after the Pion agent has already prepared local host candidates, the NAT layer may return those partial local candidates so the strategy can append operator endpoint hints and start connectivity checks; private host candidates are still filtered before publication. This plan may use a more aggressive ICE check interval, a binding-request budget scaled and burst-amplified within a hard cap from `nat.connect_timeout`, shorter srflx/prflx acceptance waits, and a failed-check timeout that is not shorter than `nat.connect_timeout`, so it behaves closer to sustained UDP punching without changing session or solver interfaces. Session candidate execution budget must scale with per-candidate execution timeout and candidate count so `direct_prefer`, split `public_direct` hint attempts, and relay fallback each get a real attempt window. During that ICE check window, if an incoming STUN Binding Request creates or validates a public peer-reflexive remote candidate pair, the NAT layer may switch to that public direct pair only when the local side is either public or an RFC1918 host NAT base and the remote side is public; relay, remote private, local or remote CGNAT/overlay, loopback, link-local, multicast, and benchmark addresses must not trigger this switch. Public-direct candidate and failure observations should expose endpoint-hint local-base diagnostics, including local base count and fixed local port when available, so operators can confirm whether the advertised public hint is tied to a concrete local UDP punching socket.
- `legacyice/relay_only`: force relay-only candidate gathering and candidate selection.

Public-direct candidate signaling and same-socket pre-punch use the same bounded limit policy: ordinary candidate bursts stay capped at 1024 candidates per round, while endpoint-hint/window candidates may raise that round limit up to 4096 so a complete bounded hint window is not truncated before both peers have a chance to punch it. Retry rounds should resume from the previous round's successful send/punch offset rather than restarting at the beginning of the ordered candidate list; this prevents repeated low-offset coverage when the control plane or UDP punch loop times out mid-window. Same-socket remote-candidate pre-punch should continue for bounded retry rounds during the executor lifecycle, scaled by `nat.connect_timeout`, so a single early offer/answer exchange is not the only NAT opening attempt. A valid `candidate` message that arrives before remote ICE credentials are available should still trigger immediate same-socket pre-punch before it is buffered; it must not be added to the ICE agent or start ICE connect until the credentials arrive. Each pre-punch round may send a small bounded packet burst per candidate from the same public-direct socket to better match natpierce-style short-window punching without turning the default path into an unbounded UDP flood. Candidate signal and punch observations should expose the current offset, next offset, first/last candidate address, covered port range, punch socket local address, per-candidate burst, and actual packet count so live debugging can compare WinkYou's actual coverage with independently observed endpoints. Larger or unbounded port sweeps remain out of scope for the default production path.
...
- `connectivity.mode=auto`: use configured strategy order, defaulting to `legacy_ice_udp` -> `relay_only` -> `signal_relay`
...
- `tcp_framed.role=auto|listen|dial`: optionally pins which side listens or dials for explicit TCP testing
- `tcp_framed.dial_addr`: optional static TCP endpoint for a forced dialer
- `signal_relay`: may be listed explicitly as a coordinator-dependent fallback; default production order keeps it after `legacy_ice_udp` and `relay_only`
...
When the initial bound path is `signal_relay`, session should bind it promptly instead of waiting for all protected-direct attempts to fail, because the coordinator-dependent relay path is the only currently successful data plane. Later protected-direct improvement attempts must not clear the bound path merely because their temporary executor fails. Message-backed bound transports must continue receiving their strategy messages while another strategy executor is active.
...
Client runtime may publish explicit backend routes for operator-managed gateway peers through coordinator metadata using `route:<cidr>` endpoint entries derived from `node.advertise_routes`. Other peers may bind those published CIDRs into the gateway peer's WireGuard `AllowedIPs` and add local routes via the gateway peer virtual IP. Windows TUN routes should be installed with an explicit low route/interface metric so same-prefix external overlay routes are less likely to override a bound WinkYou backend route; more specific host routes still win by normal OS longest-prefix matching and must be removed or matched by an equally specific advertised route. This is a narrow routing feature for known backend networks, such as an `<NODE_A_HOST>` subnet behind a WinkYou peer; it is not automatic peer relay, does not move path selection into `pkg/tunnel`, and does not make the solver aware of route ownership. The gateway host remains responsible for OS IP forwarding and firewall policy.
...
- `signal_relay` is explicitly coordinator-dependent: if `<NODE_B_HOST>` is both the coordinator reachability path and the underlay carrying the coordinator signal stream, disconnecting it will break the `signal_relay` data path too. A session can survive only when at least one bound `PacketTransport` remains reachable without the disconnected underlay.
...
Real-node validation on 2026-06-06 showed `signal_relay` binding `<NODE_B_INSTANCE>` to `<NODE_A_INSTANCE>` with `Path Strat: signal_relay`, `Path Plan: signalrelay/coordinator_signal`, and `Path Deps: coordinator:...:coordinator_signal_stream`; failed `tcp_framed` and `legacy_ice_udp` protected-direct improvement attempts no longer cleared that bound path. The same validation showed that local access to `<IPV4_1>:22` through natpierce/`<NODE_B_HOST>` did not imply arbitrary endpoint reachability: a temporary TCP listener on `<NODE_A_HOST>` random port did not receive local connections, and `tcpdump -i natpierce tcp port 22` saw SSH traffic sourced from `<IPV4_2>` rather than the local `<IPV4_3>`. This must be treated as external-overlay or forwarded-port evidence, not proof of a protected direct WinkYou underlay. The same run also showed a separate Windows TUN issue: in-band `33435` and other already-read virtual packets traversed the tunnel, but external `wink ping` / PowerShell UDP probes to `<IPV4_4>:33434` could increment `wink0` `OutboundDiscardedPackets` without appearing in the wink process TUN read trace or on remote `tcpdump`. The default-off `WINKYOU_TRACE_TUN_PACKETS=1` switch exists to separate OS/Wintun ingress failures from solver or `PacketTransport` failures.

NAT/ICE candidate filtering and public candidate hints are owned by the NAT/legacy ICE boundary. Config supports candidate interface include/exclude, candidate CIDR include/exclude, candidate UDP port range, optional Pion ICE NAT1To1 public candidate hints, public endpoint hints, runtime public endpoint hints from STUN mapping observations, bounded public endpoint hint port-window expansion, and explicit trusted direct CIDRs under `nat.*`; these user filters and hints are passed into the pion ICE agent or the legacy ICE strategy boundary. `legacyice/public_direct` applies automatic overlay/virtual interface exclusion only when the operator has not provided an explicit interface include list; this keeps protected-direct proof conservative by default while still allowing controlled reproduction on a named interface such as natpierce. Explicit `nat.candidate_cidr_include` is an allow-list for public-direct candidate participation, not a trust declaration: candidates, manual endpoint hints, and runtime STUN-derived endpoint hints in that CIDR may be advertised, accepted, and selected, but path dependency classification must still treat non-public addresses as dependent unless they are also covered by `nat.direct_trusted_cidrs`. If mapped `public_endpoint_hints` also provide local base addresses, those `/32` bases are merged with the configured candidate CIDR include list instead of replacing it. The `legacyice/public_direct` plan asks the NAT layer to skip TURN/relay candidate gathering, but it does not add its own agent-level CIDR excludes, because that can prevent STUN server-reflexive candidate gathering on common private local interfaces. Instead, it filters the local candidates it advertises and filters remote candidates before handing them to the ICE agent. Candidate gather/filter diagnostics are strategy observations (`candidate_gathered` and `remote_candidates_filtered`) with total/kept/rejected counts, rejection reasons, and bounded kept/rejected candidate samples. Samples should preserve related/base addresses when available, for example `srflx:public_ip:public_port<-local_ip:local_port`, so operators can distinguish missing public candidates from filtered overlay candidates and compare WinkYou's mapped endpoint with a natpierce-observed endpoint. `nat.direct_trusted_cidrs` is intentionally narrow: it lets operators test a proven non-public underlay CIDR without globally weakening the default protected-direct rules. `nat.public_direct_trusted_cidrs` remains accepted for compatibility and is merged with `direct_trusted_cidrs`. Manual endpoint hint validation, runtime STUN-derived endpoint hints, and recent observation-derived endpoint hints must honor the public-direct allow-list, while still rejecting non-public hints by default. `wink doctor` may probe the effective public-direct STUN sources, including configured STUN servers and UDP TURN URLs derived as unauthenticated STUN binding endpoints on the same host/port, reuse a single local UDP socket across those probes, report whether mapped endpoints are stable, symmetric, or unclassified, render observed `nat.public_endpoint_hints` candidates when they are useful for operator comparison, and display the effective hint port window when it differs from the stored config. `nat.auto_public_endpoint_hints` is enabled by default so startup observations become best-effort production hints; the client should also refresh those runtime hints with a short timeout immediately before creating a new peer session and before protected-direct improvement, preserving the previous hints if the refresh fails or if the refresh succeeds without usable endpoint hints, so public-direct attempts do not lose the last viable short-window mapping clue. The client may also seed the next strategy build from recent local `legacyice/public_direct` `candidate_gathered` srflx samples, bounded by age and count, so an observed mapping can influence the next punch attempt without being treated as proof. `nat.public_endpoint_hint_port_window` defaults to a small bounded window for predictable port drift; if the startup or refreshed NAT type is symmetric or unclassified/unknown and endpoint hints exist, production raises the effective window to a controlled wider value to cover more endpoint-dependent drift without changing the stored config. Symmetric, endpoint-dependent, or unclassified mappings must remain labeled as unproven rather than treated as stable. When those hints exist, the legacy ICE ranker may run `legacyice/public_direct` before ordinary `legacyice/direct_prefer` to avoid losing the mapped endpoint timing window. Operators may disable the automation or set the window to zero for conservative diagnostics. Doctor may also read persisted observation history to summarize `legacyice/public_direct` evidence for operators. Those diagnostics remain outside `pkg/session` and `pkg/solver`. NAT1To1 hints are operator-supplied external/local IP mappings for stable public mappings; they are not a replacement for STUN when the NAT rewrites UDP ports per socket. `pkg/session` and `pkg/solver` must remain unaware of interface, CIDR, port-range, NAT1To1, public endpoint hint, trusted CIDR, or candidate rejection details.
...
- `tcp_framed.role` and `tcp_framed.dial_addr` allow fixed listener/dialer lab setups without changing `PacketTransport`
