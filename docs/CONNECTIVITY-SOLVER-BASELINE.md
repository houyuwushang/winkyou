...
Public-direct candidate signaling and same-socket pre-punch use the same bounded limit policy: ordinary candidate bursts stay capped at 1024 candidates per round, while endpoint-hint/window candidates may raise that round limit up to 4096 so a complete bounded hint window is not truncated before both peers have a chance to punch it. Same-socket remote-candidate pre-punch should continue for bounded retry rounds during the executor lifecycle, scaled by `nat.connect_timeout`, so a single early offer/answer exchange is not the only NAT opening attempt. Larger or unbounded port sweeps remain out of scope for the default production path.
