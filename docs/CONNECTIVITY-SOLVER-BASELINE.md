...
Real-node validation on 2026-06-06 showed `signal_relay` binding local-live to inner-live with `Path Strat: signal_relay`, `Path Plan: signalrelay/coordinator_signal`, and `Path Deps: coordinator:...:coordinator_signal_stream`; failed `tcp_framed` and `legacy_ice_udp` protected-direct improvement attempts no longer cleared that bound path. The same validation showed that local access to `10.6.22.1:22` through natpierce/chen-win did not imply arbitrary endpoint reachability: a temporary TCP listener on inner-gw random port did not receive local connections, and `tcpdump -i natpierce tcp port 22` saw SSH traffic sourced from `10.6.22.4` rather than the local `10.6.22.3`. This must be treated as external-overlay or forwarded-port evidence, not proof of a protected direct WinkYou underlay. The same run also showed a separate Windows TUN issue: in-band `33435` and other already-read virtual packets traversed the tunnel, but external `wink ping` / PowerShell UDP probes to `10.88.0.8:33434` could increment `wink0` `OutboundDiscardedPackets` without appearing in the wink process TUN read trace or on remote `tcpdump`. The default-off `WINKYOU_TRACE_TUN_PACKETS=1` switch exists to separate OS/Wintun ingress failures from solver or `PacketTransport` failures.
