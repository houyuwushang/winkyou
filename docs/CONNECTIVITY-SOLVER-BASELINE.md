...
- `legacyice/public_direct`: gather only host/server-reflexive direct candidates, skip TURN/relay gathering, then advertise only public direct candidates and ignore ambiguous remote candidates, including private ranges, `100.64.0.0/10`, loopback, link-local, multicast, and benchmark/overlay ranges, before attempting a public direct ICE path. This plan may use a more aggressive ICE check interval, more binding requests, and shorter srflx/prflx acceptance waits than the generic plan, bounded by `nat.connect_timeout`, so it behaves closer to sustained UDP punching without changing session or solver interfaces. During that ICE check window, if an incoming STUN Binding Request creates or validates a public peer-reflexive remote candidate pair, the NAT layer may switch to that public direct pair; relay, private, CGNAT/overlay, loopback, link-local, multicast, and benchmark addresses must not trigger this switch.
