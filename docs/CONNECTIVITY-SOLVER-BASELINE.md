...
- `legacyice/public_direct`: gather only host/server-reflexive direct candidates, skip TURN/relay gathering, then advertise only public direct candidates and ignore ambiguous remote candidates, including private ranges, `100.64.0.0/10`, loopback, link-local, multicast, and benchmark/overlay ranges, before attempting a public direct ICE path. This plan may use a more aggressive ICE check interval and more binding requests than the generic plan, bounded by `nat.connect_timeout`, so it behaves closer to sustained UDP punching without changing session or solver interfaces.
