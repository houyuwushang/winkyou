...
- 2026-06-06 起，显式 `nat.candidate_cidr_include` 会让 `legacyice/public_direct` 接受该 CIDR 内的非公网 host/peer-reflexive 候选参与连接尝试，并会与 mapped `public_endpoint_hints` 的本地 base `/32` 合并使用，避免 endpoint hint 覆盖受控 underlay include；它不会自动消除 path dependency，只有 `nat.direct_trusted_cidrs` 才表示该 underlay 可作为 protected-direct 证据。
