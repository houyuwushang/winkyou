...
- 2026-06-06 起，生产 resolver 在 `connectivity.mode=auto` 且本机检测为 `nat_type=symmetric` 时，会优先尝试 `relay_only`，再保留 `legacy_ice_udp`。这不会设置 `ForceRelay`，因此 legacy 内部的 `public_direct` 仍可作为后续 fallback/improvement 继续争取独立路径。
