...
- 2026-06-06 起，`legacyice/public_direct` 会在 offer/answer 后发送三轮短间隔、有界 `candidate` 信令 burst，并通过 `candidate_signaled` observation 暴露发送数量、总数、round/rounds 和是否 capped。candidate 消息如果乱序先到，只会缓存远端候选，不会在 remote ICE credentials 未到时提前启动 connect。这让 coordinator 和已建立虚拟网内的 `session_signal` 都能多保留候选信令，降低短窗口打洞时单次候选消息丢失或延迟的影响。
...
- 2026-06-06 起，生产配置在启动 STUN mapping 判定为 symmetric/endpoint-dependent 且本轮存在 `public_endpoint_hints` 时，会把 effective `public_endpoint_hint_port_window` 从默认 `2` 提高到 `16`；显式 `0` 仍表示关闭，用户配置大于 `16` 不会被降低。这个改动用于覆盖 natpierce 类工具常见的更宽端口漂移探测，但仍不会把 endpoint-dependent STUN 映射当作稳定 proof。
- 2026-06-06 起，`wink doctor` 的 `candidate filters` 会显示 symmetric NAT + endpoint hints 下的 `effective_public_endpoint_hint_port_window=16`，并让 observed endpoint hint 逻辑与生产 `candidate_cidr_include`/trusted CIDR allow-list 保持一致，避免排查时误把配置默认值 `2` 当成实际执行窗口。
- 2026-06-06 起，如果启动 STUN mapping 只有一个来源成功、NAT 类型仍为 `unknown`，但本轮已经生成 runtime/public endpoint hint，生产配置也会把 effective `public_endpoint_hint_port_window` 提高到 `16`，doctor 会显示 `effective_window_reason=unclassified_nat_endpoint_hints`。这覆盖了“证据不足以证明稳定映射”的常见自托管/单 STUN 场景，继续按 best-effort 打洞处理。
...
- 2026-06-06 起，client 创建新的 peer session 前，以及已 bound 的 relay/依赖路径发起 protected-direct improvement 前，会用短超时刷新 runtime STUN endpoint hint；刷新成功会更新 NAT 类型与 runtime hint，刷新失败则保留上一轮 hint。生产 strategy factory 也改为每次 build strategy 时读取当前 hint，避免 protected-direct improvement 继续使用 session 创建时冻结的旧公网映射。
