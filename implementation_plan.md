...
- 2026-06-06 起，`legacyice/public_direct` 会在 offer/answer 后发送一轮有界 `candidate` 信令重发，并通过 `candidate_signaled` observation 暴露发送数量、总数和是否 capped。candidate 消息如果乱序先到，只会缓存远端候选，不会在 remote ICE credentials 未到时提前启动 connect。这让 coordinator 和已建立虚拟网内的 `session_signal` 都能多保留一份候选信令，降低短窗口打洞时单次候选消息丢失或延迟的影响。
