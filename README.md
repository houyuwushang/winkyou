...
同理，`nat.candidate_cidr_include` 现在会让 `legacyice/public_direct` 接受该 CIDR 内的 host/peer-reflexive 候选参与连接尝试，适合受控复现 natpierce 或路由器日志中看到的非公网 underlay。它不会自动把路径标为 `protected_direct`；未进入 `nat.direct_trusted_cidrs` 的非公网候选仍会在 path summary 中保留 dependency。
