...
- `candidate_failed`：`public_direct` 失败时会附带最近一次 `last_local_candidate_*` / `last_remote_candidate_*` / `last_signal_*` 摘要、`public_endpoint_hint_count`、`ice_state`、`ice_remote_candidate_count` 和 `ice_selected_pair`，便于直接判断是本端没有发布有效候选、远端候选被过滤，候选信令是否实际发出，还是 ICE checks 已经进入 checking 但没选中路径。
